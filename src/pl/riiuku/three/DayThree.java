...
public class DayThree {
    ...
    public void doIt() {
        Map<Point, Signal> spaceV1 = new HashMap<>();
        Map<Point, Signal> spaceV2 = new HashMap<>();
        String[] fw = firstWire.split(",");
        String[] sw = secondWire.split(",");
        int pointX = 0;
        int pointY = 0;
        spaceV1.put(new Point(pointX, pointY), new Signal("O", 0));
        spaceV2.put(new Point(pointX, pointY), new Signal("O", 0));
        int count = 0;
        for (String s : fw) {
            ...
            for (int j = 0; j < amount; j++) {
                ...
                switch (direction) {
                    case "R":
                        pointY += 1;
                        spaceV1.putIfAbsent(new Point(pointX, pointY), new Signal("-", count));
                        break;
                    case "L":
                        pointY -= 1;
                        spaceV1.putIfAbsent(new Point(pointX, pointY), new Signal("-", count));
        ...
        for (String s : sw) {
            ...
            for (int j = 0; j < amount; j++) {
                ...
                switch (direction) {
                    case "R":
                        pointY += 1;
                        spaceV2.putIfAbsent(new Point(pointX, pointY), new Signal("-", count));
                        break;
                    case "L":
                        pointY -= 1;
                        spaceV2.putIfAbsent(new Point(pointX, pointY), new Signal("-", count));
        ...
        spaceV1.forEach((key, value) -> {
           if(spaceV2.containsKey(key)) {
               Signal signal = spaceV2.get(key);
               if((value.type.equals("-") && signal.type.equals("|")) || (value.type.equals("|") && signal.type.equals("-")) ) {
                    result.add(value.steps + signal.steps);
               }
           }
        });
...
class Signal {
    ...
    public Signal(String type, int steps) {
        this.type = type;
        this.steps = steps;
    }
}
